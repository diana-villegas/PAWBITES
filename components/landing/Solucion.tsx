      ...
      >
        ...
        <motion.div variants={item} className="mt-4">
          <Hairline surface="bg" className="w-fit">
            <span className="block px-4 py-2 text-[16px] font-semibold">
              <Accent>{mecanismo}</Accent>
            </span>
          </Hairline>
        </motion.div>

        <motion.p variants={item} className="mt-5 max-w-2xl text-[16px] leading-relaxed text-[var(--text-secondary)]">
        ...
        <ol className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-3">
          {pasos.map((p, i) => (
            <motion.li key={i} variants={item} className="flex items-start gap-4 md:flex-col">
              <span
                aria-hidden="true"
                className="flex size-11 shrink-0 items-center justify-center rounded-[var(--radius-button)] border border-[color-mix(in_oklab,var(--accent)_22%,transparent)] bg-[var(--chip-bg)] text-[16px] font-bold tabular-nums text-[var(--accent)]"
              >
                {String(i + 1).padStart(2, '0')}
              </span>
              <div className="pt-1 md:pt-0">
                <h3 className="text-[16px] font-semibold text-[var(--text-primary)]">{p.titulo}</h3>
                <p className="mt-1 text-[16px] leading-snug text-[var(--text-secondary)]">{p.detalle}</p>
              </div>
            </motion.li>
          ))}
        </ol>
