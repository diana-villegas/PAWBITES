...
export interface OfertaProps {
  ...
  mensual: PlanOferta;
  /** Slot opcional dentro de la card Anual, entre features y CTA — para reforzar
   * el dispositivo ownable de FICHA-ARTE.md u otra prueba visual del mecanismo. */
  extraAnual?: ReactNode;
...
function TrialBadge({ dias }: { dias: number }) {
  return (
    <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-[color-mix(in_oklab,var(--accent)_13%,transparent)] px-2.5 py-1 text-[12px] font-bold uppercase tracking-[0.06em] text-[var(--accent)]">
  ...
  return (
    <div>
      <p className="flex items-baseline gap-1">
        <span className="text-4xl font-bold leading-none tabular-nums text-[var(--text-primary)] [font-family:var(--font-display)]">
          {plan.precioMes}
        </span>
        <span className="text-sm text-[var(--text-secondary)]">{plan.sufijo ?? '/mes'}</span>
      </p>
      {plan.descomposicionDia && (
        <p className="mt-1 text-xs text-[var(--text-secondary)]">{plan.descomposicionDia}</p>
      )}
    </div>
  );
}
...
function Features({ items, origen }: { items: string[]; origen: string }) {
  ...
  return (
    <ul className="mt-5 flex flex-col gap-3">
      {items.map((f, i) => (
        <li key={i} className="flex items-start gap-3 text-[16px] leading-snug text-[var(--text-primary)]">
          <CheckCustom />
          <span>{f}</span>
        </li>
      ))}
    ...
    <SectionShell id={id} elevacion="base" ariaLabel="Planes y precios">
      <motion.div variants={contenedor} initial="hidden" whileInView="visible" viewport={VIEWPORT_ONCE}>
        ...
        {stack && (
          <motion.div
            variants={item}
            className="mx-auto mt-8 max-w-xl rounded-[var(--radius-card)] border border-[color-mix(in_oklab,var(--text-tertiary)_25%,transparent)] bg-[var(--surface)] p-6"
          >
            <ul className="flex flex-col gap-3">
              {stack.lineas.map((l, i) => (
                <li key={i} className="flex items-start justify-between gap-4 text-[16px]">
        ...
        <div className="mx-auto mt-10 grid max-w-4xl grid-cols-1 items-start gap-6 md:grid-cols-2">
          ...
          <motion.div variants={item} className="relative md:-translate-y-2">
            {anual.badge && (
              <span className="absolute -top-2 left-1/2 z-10 -translate-x-1/2 rounded-full border border-[color-mix(in_oklab,var(--accent)_25%,transparent)] bg-[var(--accent)] px-3 py-1 text-[12px] font-bold uppercase tracking-[0.06em] text-[var(--bg)]">
                {anual.badge}
              </span>
            )}
            <Hairline emphasis surface="surface" className="shadow-[0_12px_36px_color-mix(in_oklab,var(--accent)_16%,transparent)]">
              <div className="rounded-[var(--radius-card)] bg-[color-mix(in_oklab,var(--accent)_5%,transparent)] p-6 md:p-7">
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-2 text-[16px] font-semibold text-[var(--accent)]">{anual.ahorro}</p>
                </div>
                <Features items={anual.features} origen="Oferta → anual" />
                {extraAnual && <div className="mt-5">{extraAnual}</div>}
                <div className="mt-6">
