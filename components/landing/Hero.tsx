...
export interface HeroProps {
  ...
  loginHref?: string;
  loginLabel?: string;
  /** Ancla secundaria de navegación (ej. "Ver precios" → #oferta) — atajo de
   * teclado/mouse a la oferta sin depender solo del scroll manual. */
  preciosHref?: string;
  preciosLabel?: string;
  /** Anclas adicionales para el visitante de alta intención (h7 usabilidad —
   * atajos directos en vez de depender solo del scroll manual). */
  navExtra?: { label: string; href: string }[];
  ...
  return (
    <section id={id} className="relative overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-6xl px-5">
        {/* Header 64px: marca a la izquierda, "Ver precios" + "Entrar" terciarios a la derecha (19) */}
          ...
          <nav className="flex items-center gap-1">
            {navExtra.map((n) => (
              <a
                key={n.href}
                href={n.href}
                className="hidden px-2 py-3 text-sm font-medium text-[var(--text-tertiary)] sm:inline-block"
              >
                {n.label}
              </a>
            ))}
            {preciosHref && (
              <a href={preciosHref} className="px-2 py-3 text-sm font-medium text-[var(--text-tertiary)]">
                {preciosLabel}
              </a>
            )}
            {loginHref && (
        ...
        >
          ...
          <h1 className="text-balance text-5xl font-bold leading-[1.08] tracking-[-0.01em] text-[var(--text-primary)] [font-family:var(--font-display)] md:text-7xl">
